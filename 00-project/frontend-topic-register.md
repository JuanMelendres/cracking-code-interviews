...
| F-402 | WebSocket and Server-Sent Events for Real-Time UI | ADV | Real, browser-executed evidence (Playwright/Chromium, native `WebSocket`/`EventSource`): a real closed WebSocket that never reconnects on its own, a real measured exponential-backoff reconnect wrapper, and a real EventSource that reconnects three times automatically with zero client-side retry code. | 🟢 | [`websocket-and-server-sent-events-for-realtime-ui.md`](../syllabus/21-frontend-web/websocket-and-server-sent-events-for-realtime-ui.md) |
| F-403 | Micro-Frontends and Module Federation | ADV | Real, two-build evidence (genuinely separate Webpack 5 `remote`/`host` apps, Playwright/Chromium): a real runtime fetch of the remote's code by the host, and a real rebuild-only-the-remote test proving independent deployability — the host's rendered button text and color change with the host's own bundle never rebuilt. | 🟢 | [`micro-frontends-and-module-federation.md`](../syllabus/21-frontend-web/micro-frontends-and-module-federation.md) |
