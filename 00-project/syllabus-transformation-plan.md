...
- **`T-510` filled 2026-09-11.** Same audit's `05-spring` gap: the domain still lacked a dedicated Spring Data JPA repository-abstraction chapter (derived query methods, `@Query`, Specifications) — this real, correctly-reserved Master Topic Register slot (T-510, "Spring Data repositories & query derivation," IWI 5.3, Core tier, never written) had been left open when `T-518` (Bean Validation and Global Exception Handling) closed the domain's other same-day gap. Closed with [Spring Data JPA Repository Abstraction](../syllabus/05-spring/spring-data-jpa-repository-abstraction.md), backed by a real app (`practice/java/spring-data-jpa-repositories/`, Spring Framework 6.2.19 + Spring Data JPA 3.5.13 + Hibernate ORM 6.6.55.Final + H2) proving a derived query method's real generated SQL, a real second, independent `COUNT` query behind `Pageable`, a real JPQL join query and a real native `@Query` bound to a zero-boilerplate interface projection, a real `Specification` composing 0/1/2 predicates at runtime, and — the marquee finding — a real `ApplicationContext` startup failure (`org.springframework.data.mapping.PropertyReferenceException`) from a deliberately typo'd derived-method property name, proving Spring Data validates a method name's property path against the entity's real JPA metamodel eagerly at repository-proxy creation, not lazily on first call. `05-spring`'s full 22-domain-audit gap list is now closed.
...
- **`F-401` assigned 2026-09-11**, opening a new "D-F4 · Advanced Frontend Architecture & Security" tier in `00-project/frontend-topic-register.md` (this domain's own `F-xxx` numbering is independent of the backend's `T-xxx` global ID space; extended per the register's own stated precedent, F-001–F-003). Same audit's `21-frontend-web` gap: this domain's 40 originally-registered topics had zero dedicated coverage of frontend security, despite the backend's own `12-security` domain covering the equivalent server-side ground. WebSocket/real-time (closed same day, `F-402`, see below) and micro-frontends, also flagged by the same audit for this domain, were the two remaining open sub-items; micro-frontends remains open. Closed with [Frontend Security: XSS, CSRF, and Content Security Policy](../syllabus/21-frontend-web/frontend-security-xss-csrf-and-csp.md), backed by a real demo (`practice/frontend/frontend-security-xss-and-csp/`, a real Node HTTP server driven by real headless Chromium via Playwright, not jsdom) proving a real reflected-XSS payload genuinely executes through naive, unescaped server-side templating, the identical payload rendered inert once HTML-escaped, and a real CSP header (`script-src 'self'`) genuinely blocking an identical inline `<script>` with Chromium's own real console violation message captured verbatim.
- **`F-402` assigned 2026-09-11**, continuing the D-F4 tier opened by `F-401` above (same day). Second of the same audit's three `21-frontend-web` sub-items — no prior coverage of WebSocket or Server-Sent Events, despite both being common real-time-UI interview and production topics. Micro-frontends remains the one sub-item still open. Closed with [WebSocket and Server-Sent Events for Real-Time UI](../syllabus/21-frontend-web/websocket-and-server-sent-events-for-realtime-ui.md), backed by a real demo (`practice/frontend/websocket-and-sse-realtime/`, a real Node `ws` WebSocket server plus real headless Chromium via Playwright using native `WebSocket`/`EventSource`) proving a real WebSocket never reconnects on its own after a real close (verified over a real 1.5-second window), a real, measured exponential-backoff reconnect wrapper (301/601/1202ms actual against a 300/600/1200ms requested schedule), and a real `EventSource` reconnecting three times fully automatically with zero client-side retry code — a genuine RFC 6455 vs. WHATWG HTML spec-level contrast in who owns reconnection, both verified live via `WebFetch` before citing.
- **`F-403` assigned 2026-09-11**, continuing the D-F4 tier (same day as `F-401`/`F-402`). Third and last of the same audit's three `21-frontend-web` sub-items — this closes the domain's full 2026-09-11 gap-audit finding. No prior coverage of micro-frontends or Module Federation, despite the pattern being a real Staff-level frontend/platform-architecture interview topic. Closed with [Micro-Frontends and Module Federation](../syllabus/21-frontend-web/micro-frontends-and-module-federation.md), backed by a real demo (`practice/frontend/microfrontends-module-federation/`, two genuinely separate Webpack 5 builds — a `remote` exposing one component via `ModuleFederationPlugin`, a `host` consuming it at runtime — driven by real headless Chromium via Playwright) proving a real, captured browser network request showing the host fetches the remote's code live at page load rather than bundling it, and a real, direct independent-deployability proof: rebuilding only the remote (the host's own bundle never rebuilt or re-served) changes the host page's rendered button text and computed CSS color on reload. Both Webpack's own Module Federation documentation and Martin Fowler's own micro-frontends article (its core definition and named downsides — payload bloat, environment mismatches, operational complexity) were verified live via `WebFetch` before citing.
