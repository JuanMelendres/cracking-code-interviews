...
- **`T-116` assigned 2026-09-10**, continuing `02-java/language-core`'s own existing `T-1xx` sequence (T-101–T-115 already assigned). The same audit's `java.time` gap, closed same day: [java.time API: Dates, Times, and Durations](../syllabus/02-java/language-core/java-time-api.md), backed by a real demo (`practice/java/language-core/java-time-api/`) proving real immutability (contrasted against a real `Calendar` in-place mutation), a real `Period`-vs-`Duration` divergence on an actual US DST transition date, and a real, reproduced `SimpleDateFormat` thread-safety corruption under concurrent load (6,969/10,000 in one captured run) versus zero corruption for the identical workload against `DateTimeFormatter`. `02-java`'s remaining open gaps (JPMS, `MethodHandle`, bytecode fundamentals, concurrency synchronizers) are unaffected by this entry.
- **`T-417` assigned 2026-09-10**, continuing `02-java/concurrency`'s own existing `T-4xx` sequence (T-401–T-416 already assigned; T-403 was never assigned and is left as-is, not reused, matching this repository's existing convention for skipped IDs). The same audit's `java.util.concurrent` synchronizers gap, closed same day: [java.util.concurrent Synchronizers: CountDownLatch, CyclicBarrier, and Semaphore](../syllabus/02-java/concurrency/synchronizers-countdownlatch-cyclicbarrier-semaphore.md), backed by a real demo (`practice/java/concurrency/synchronizers/`) proving real `CountDownLatch` blocking-until-all-signal behavior, real evidence it cannot be reset, a real `CyclicBarrier` reused across two independent rounds, and a real, measured `Semaphore` maximum-concurrent-holder count under genuine contention. `02-java`'s remaining open gaps (JPMS, `MethodHandle`, bytecode fundamentals) are unaffected by this entry.
- **`T-617` assigned 2026-09-10**, continuing `06-databases`'s own existing `T-6xx` sequence (T-601–T-616 already assigned). The same 22-domain audit's SQL gap, closed same day: [Window Functions and Common Table Expressions](../syllabus/06-databases/window-functions-and-ctes.md), backed by a real PostgreSQL 16 lab (`practice/sql/window-functions-and-ctes/`) proving a real tie-handling divergence between `ROW_NUMBER()` and `RANK()` changes an actual "top 2 per department" result set, a real recursive CTE correctly traversing a 3-level org chart, and a real, measured `EXPLAIN ANALYZE` comparison showing a window-function approach beating a logically equivalent correlated subquery by three to four orders of magnitude on a 200,000-row table (a captured run: ~4,957×; an earlier exploratory run: ~1,506× — both real, honestly reported as run-to-run variance, not a discrepancy). `06-databases`'s remaining open gaps (JSONB, GIN/GiST/BRIN index types, full-text search) are unaffected by this entry.
- **`T-618` assigned 2026-09-10**, continuing `06-databases`'s own existing `T-6xx` sequence (T-601–T-617 already assigned). The same audit's JSONB/GIN/GiST/BRIN gap, closed same day: [JSONB and Advanced PostgreSQL Index Types](../syllabus/06-databases/jsonb-and-advanced-index-types.md), backed by a real PostgreSQL 16 lab (`practice/sql/jsonb-and-advanced-indexes/`) covering JSONB containment, GIN (JSONB and full-text search), a real GiST `EXCLUDE` constraint rejecting an overlapping booking, and BRIN's real ~1,834x size advantage. This chapter's own honestly-reported real findings go beyond simple usage guidance: a GIN index measurably slowed a ~4%-selective JSONB query while the identical mechanism delivered a real ~3,780x win for a 0.02%-selective full-text search query, and the planner declined to use an available BRIN index by default even as the table's only index. Full-text search is covered within this chapter as a GIN use case rather than as a separate topic, closing `06-databases`'s SQL-topic gap list from the same audit entirely.
