...
Thirty-three cheat sheets exist so far, ranked by each canonical chapter's own stated IWI (interview-weight-index) — built across six bounded batches, per `CLAUDE.md`'s instruction against generating an entire deliverable in one operation. 42 chapters remain uncovered.
...
Chapters are ranked by the IWI each canonical chapter states in its own "Topic register" line — this is more reliable than `00-project/knowledge-architecture-blueprint.md`'s Master Topic Register table, which predates several newer system-design/T-8xx/T-9xx chapters and does not list them at all (a known staleness gap; see the flagged coverage-audit refresh task). Built across six batches (top 8, then 5, then 5, then 5, then 5, then 5), spanning 10 domains so far (system-design, architecture, databases, spring, kafka, concurrency, collections, jvm, testing, cloud — cloud newly represented in batch 6) rather than front-loading one. Tie-breaks so far, all resolved by cross-repository reference count rather than arbitrarily: Clean/Hexagonal Architecture over DDD Tactical Design in batch 4 (DDD Tactical closed in batch 5); Storage Selection Trade-offs over Performance Methodology/SLO Error Budgets (15 vs. 10 refs) in batch 5 (Performance Methodology closed in batch 6, alongside its IWI-6.90 tie-partner Logging/Metrics/Tracing/OTel, which needed no tie-break since both fit within batch 6's top 5); Percentiles/Tail Latency over Deadlock/Race-Conditions/Thread-Diagnostics (14 vs. 10 refs) for batch 6's 5th slot. Next verified candidate for a future batch: `handbook/concurrency/deadlock-race-conditions-and-thread-diagnostics.md` (T-409, IWI 6.70, batch 6's tie-break loser) — re-run the scripted IWI scan across all 75 chapters' own Topic register lines before selecting the rest of that batch, since the ranking shifts as chapters get covered.
