...
- The JDK's own `Collections` framework overview, and [Storage Selection Trade-offs](../system-design/storage-selection-tradeoffs.md) for the analogous access-pattern-first method applied one layer up, at the storage-technology level rather than the in-memory-collection level.
