...
- **Phase 7 (Deprecation of old paths)** — removing redirect stubs at old `handbook/<domain>/` paths — is the only genuinely destructive phase and has not started; it requires its own explicit, separate approval per the plan's own governance. It is now the only unstarted phase besides ongoing Phase 5 gap-filling (one known chapter, `18-engineering-practices/git-internals-and-collaboration-workflows.md`, still L3/L4 only).
