...
>
> **Gap found and closed: Vector Clocks and Quorum-Based Replication (2026-09-10, same day).** The one remaining gap this domain's own Consensus Algorithms entry flagged as open. Closed with [Vector Clocks and Quorum-Based Replication](vector-clocks-and-quorum-based-replication.md) (T-2407, a genuinely new topic — absent from the original Master Topic Register entirely), backed by a real demo (`practice/java/vector-clocks-and-quorum-replication/`) proving two things directly: a real vector-clock comparison correctly distinguishes a genuine concurrent write conflict (`CONCURRENT`) from a causal read-then-write supersession (`DOMINATES`); and a real, exhaustive enumeration of every possible write-quorum/read-quorum pair for a 5-replica set proves Dynamo's `W + R > N` rule holds in all 100 cases when the sum exceeds `N`, while finding a real, named counterexample the moment the sum only equals `N` — directly correcting the common `W + R = N` "close enough" misconception.
