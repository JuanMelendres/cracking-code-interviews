...
- Pair any Proxy-level auth gate with a REAL authorization check inside the destination Route Handler or Server Action — Proxy's own docs call this out explicitly, and this repository's [AuthN/AuthZ chapter](../12-security/authn-authz-rbac-vs-abac.md) covers the authoritative-check side of that pairing in depth.
...
- [AuthN/AuthZ: RBAC vs. ABAC](../12-security/authn-authz-rbac-vs-abac.md) — the backend-domain chapter covering the AUTHORITATIVE authorization check this chapter's Proxy-level gate is explicitly scoped as only a fast, optimistic complement to, never a replacement for.
