...
- [OWASP Top 10 for Backend Services](../12-security/owasp-top-10-for-backend-services.md) — the backend-domain chapter covering broken access control generally; this chapter's own curl-bypasses-CORS finding is a concrete instance of "don't rely on client-enforced controls."
- [AuthN/AuthZ: RBAC vs. ABAC](../12-security/authn-authz-rbac-vs-abac.md) — the backend-domain chapter this chapter's own shared-secret-vs-session distinction (service-to-service auth vs. user auth) connects to directly.
