...
domain: 12-security
status: 10 of 10 chapters written — domain gap-audited and closed (2026-09-10); enterprise SSO/SAML added 2026-09-14, fully L1-L4
last_updated: 2026-09-14
---

# Security

OWASP Top 10, OAuth2/OIDC/JWT, enterprise SSO/SAML/federated identity, secrets management, multi-tenancy isolation, supply-chain risk, and CSRF/CORS/session security. Existing `handbook/security/` (8 chapters) relocated here unchanged in content; a ninth chapter closed a real audited gap; a tenth chapter added real-world enterprise I&AM coverage (SAML, PingFederate/Okta/Azure AD/Keycloak) at direct user request.
...
>
> **Enterprise SSO/SAML added (2026-09-14), user-requested.** Real, direct gap in the domain versus a real job-posting requirement the user asked about explicitly: "strong understanding of the I&AM landscape — SSO, Federated SSO, SAML, OAuth2, OIDC — and experience with commercial IAM solutions like PingFederate." OAuth2/OIDC were already covered in depth (`oauth2-oidc-and-jwt.md`); SSO, Federated SSO, SAML, and the commercial IAM vendor landscape (PingFederate, Okta, Azure AD/Entra ID, Keycloak, ADFS) had zero coverage anywhere in the repository. Closed with a tenth chapter, [Enterprise SSO: SAML, Federated Identity, and Commercial IAM Integration](enterprise-sso-saml-and-federated-identity.md) (T-1309) — SAML 2.0 core artifacts, a precise SAML/OAuth2/OIDC comparison table, the commercial IAM landscape table, and real, live-verified (not locally executed — stated explicitly, the same honesty discipline `oauth2-oidc-and-jwt.md` already uses for its own OAuth2/OIDC flow section) Spring Security `spring-security-saml2-service-provider` configuration: the exact `pom.xml` dependency, the current `spring.security.saml2.relyingparty.registration.*` YAML structure (verified live against Spring Security's own reference docs), and a `SecurityConfig` wiring example — continuing directly from `spring-mvc-fundamentals.md`'s own `pom.xml`/`application.yml` anatomy section.
