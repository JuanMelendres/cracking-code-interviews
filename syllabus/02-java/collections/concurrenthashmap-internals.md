...
Concurrent `put()` calls from multiple threads on a plain `HashMap` can corrupt its internal bucket structure, losing entries — with no exception, just a wrong final size. On the JDK 7 (and earlier) `HashMap` implementation, the same unguarded concurrent access during a resize could go further than lost entries: it could produce a genuine infinite loop. This is a specific, mechanical hazard — not a vague "HashMap isn't thread-safe" — and it is worth being precise about it, because it is commonly and incorrectly conflated with an out-of-memory condition or blamed on the garbage collector, when the actual cause is neither: the JVM's memory is fine and the GC is doing nothing wrong; a CPU core is spinning at 100% inside `get()`/`put()`, forever, because the map's own internal bucket structure has become circular. **Why it happens:** JDK 7's `HashMap` resizes by rehashing every entry into a new, larger bucket array using a linked list per bucket, and its resize implementation *reversed* the order of each bucket's linked list as it moved entries — safe under single-threaded access, but if two threads both trigger a resize on the same map concurrently, one thread's partially-completed pointer rewiring can interleave with another's, producing a node whose `next` pointer points back to an earlier node in the same list — a cycle. Once that cycle exists, any subsequent `get()` or `put()` that has to walk that bucket's chain loops forever, pinning one CPU core at 100% with no exception, no OutOfMemoryError, and no GC pause to point at — the map is not leaking memory, it is stuck walking a circular linked list that will never end. **This was fixed, not merely mitigated, in JDK 8**: the redesigned resize (and treeification) logic no longer reverses bucket order during a rehash, so the specific pointer-cycle mechanism cannot occur on JDK 8+ even under concurrent, unsynchronized access — but JDK 8+'s plain `HashMap` is still not thread-safe: the *silent corruption / lost-entry* failure mode above remains real and unfixed, because it stems from a different root cause (two threads racing to write the same bucket slot, not the resize-reversal bug). The practical interview point: know which JDK generation each failure mode belongs to, and never present the JDK 7-specific infinite loop as "what happens" on a modern JDK — the correct modern answer is silent corruption, not a livelock.
