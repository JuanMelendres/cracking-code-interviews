...
**`default` and `static` interface methods, precisely.** A `default` method (introduced Java 8) carries a real body directly on the interface — any implementing class inherits that body automatically and only needs to override it if it wants different behavior; this is exactly what let the Java Collections Framework add new methods (like `List.sort()`) to existing interfaces without breaking every class that had already implemented them, since old implementers simply inherit the new default rather than being forced to implement it. A `static` interface method also carries a real body, but belongs to the *interface itself*, never to an implementing class or instance — it is called as `InterfaceName.method()` only, the same way `Comparator.comparing(...)` is called directly on `Comparator`, never through some object that implements it. Section 7's first demo proves both directly: a real `MathOps.square(5)` static call, and a real `Calculator` that inherits `MathOps`'s `cube()` default method without writing a line of code for it.
...
**`default`/`static` interface methods, and Java's real diamond-problem collision** — added 2026-09-08, closing a gap found when the user asked for more depth on this exact topic: [`DefaultStaticInterfaceMethodsDemo.java`](../../../practice/java/oop-fundamentals/classes-and-objects/src/DefaultStaticInterfaceMethodsDemo.java) proves `MathOps.square(5)` (a real interface static-method call, through the interface name) and a `Calculator` that inherits `MathOps`'s `cube()` default method with zero code of its own — 2/2 assertions pass. [`BrokenDefaultMethodDiamondCollision.java`](../../../practice/java/oop-fundamentals/classes-and-objects/src/BrokenDefaultMethodDiamondCollision.java) is a real, deliberately broken `Duck implements Flyer, Swimmer` where both interfaces declare a conflicting `default move()` — it genuinely fails to compile, with `javac` reporting `class Duck inherits unrelated defaults for move() from types Flyer and Swimmer`, captured verbatim. [`FixedDefaultMethodDiamondCollision.java`](../../../practice/java/oop-fundamentals/classes-and-objects/src/FixedDefaultMethodDiamondCollision.java) resolves it with an explicit override that calls both parents' versions via `Flyer.super.move()`/`Swimmer.super.move()`, producing the real string `"flying and swimming"` — 1/1 assertion passes. Full transcripts in `default-static-interface-methods-output.txt`, `diamond-collision-compile-error.txt`, and `fixed-diamond-collision-output.txt`.
...
- An **interface's `static` method** belongs to the interface itself, not to any implementing class, and is called as `InterfaceName.method()`, never through an instance reference — a genuinely different rule from a class's own static methods, which (while discouraged, per [Polymorphism and Dynamic Dispatch Mechanics](polymorphism-and-dynamic-dispatch.md)'s own field/static-hiding coverage) can be called through an instance reference too. Proven directly, not just asserted, in Section 7's `MathOps.square(5)` demo.
- **Two interfaces with a colliding `default` method**, both implemented by the same class, is a compile error unless that class explicitly overrides the method itself — Java refuses to silently pick one interface's version over the other's. Section 7's `Duck implements Flyer, Swimmer` demo reproduces the real `javac` error this produces, then the real fix.
