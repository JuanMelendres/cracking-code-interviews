...
status: 4 of 4 mapped chapters physically relocated (Phase 3, 2026-09-03); L1/L2 retrofit complete (Phase 5, 2026-09-04) — domain fully L1-L4; 5th chapter added 2026-09-11 (Metric Cardinality and Alert Fatigue, T-2409 — gap found in a full 22-domain audit); 6th chapter added 2026-09-21 (Chaos Engineering: Fault Injection and Resilience Verification, T-2423 — gap audit)
last_updated: 2026-09-21
---

# Observability

"How do I know something is wrong in production" — logging/metrics/tracing, SLOs and error budgets, incident response, percentile/tail-latency measurement, metric cardinality/alert-fatigue management, and chaos engineering (proactive fault-injection verification). Split out of `performance/` per the plan's Section 3.3 (a different skill than JVM tuning).
...
>
> **Gap found and closed: Chaos Engineering (2026-09-21).** A further gap audit found this domain covered detection (alerting) and reaction (incident response) but had zero coverage anywhere in the repository of proactively verifying that either actually works — zero grep hits for "chaos" before this pass, and `multi-region-failover-and-disaster-recovery.md` referenced a "DR game day" once, without explaining the broader discipline it's one instance of. Closed with [Chaos Engineering: Fault Injection and Resilience Verification](chaos-engineering-fault-injection-and-resilience-verification.md) (T-2423), backed by a real, timed demo (`practice/java/observability/chaos-engineering-fault-injection/`) proving a real fault deliberately injected against only a minimized 25% canary cohort was detected by real multi-window burn-rate monitoring (the same technique T-2409 documents) in ~1.1 real seconds / 12 real requests, with real recovery confirmed ~310ms after rollback — no mocked clock anywhere in the demo.
