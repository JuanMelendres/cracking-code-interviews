...
related:
  ...
  - ../09-messaging-event-driven/delivery-semantics-and-exactly-once.md
...
### "Check before write" versus "insert/upsert by key" — naming the two implementation shapes

There are two structurally different ways to implement the mechanism above, and only one of them is actually safe:

- **Check-before-write (unsafe as a sole mechanism):** `SELECT * FROM idempotency_keys WHERE key = ?`; if no row exists, proceed to perform the side effect and then insert the key. This has exactly the same race as any read-then-write sequence: two concurrent requests carrying the same key can both execute the `SELECT`, both see no row, and both proceed to perform the side effect — the check passed for both, because neither request's check saw the other's not-yet-committed insert. A `SELECT` alone can never coordinate concurrent writers; it can only tell you the state at the moment it ran.
- **Insert/upsert by key (the correct mechanism):** attempt the write directly — either `INSERT INTO idempotency_keys (key, status) VALUES (?, 'IN_PROGRESS')` and catch the unique-violation, or an equivalent `INSERT ... ON CONFLICT DO NOTHING` / `INSERT ... ON CONFLICT (key) DO UPDATE ...` upsert — and let the database's own unique constraint decide, atomically, which request (if any) is first. This is the mechanism [Internal Implementation](#internal-implementation) demonstrates: the "check" and the "write" are not two separate steps at all, they are one atomic statement, which is exactly why it closes the race the check-before-write shape cannot.

The interview-relevant point: a candidate who says "check if the key exists, then insert" is describing the *unsafe* shape, even if they call it idempotency correctly by name — the safe implementation makes the write itself the check, via a unique constraint or an atomic upsert, never a separate read first.

### The same mechanism at a Kafka consumer, not just a REST endpoint

Everything above is framed around an HTTP request carrying an `Idempotency-Key` header, but the identical mechanism — a unique-constraint-backed store keyed by an operation identifier, written atomically rather than checked-then-written — applies just as directly to a Kafka consumer processing a message more than once (at-least-once delivery, a consumer restart after a rebalance, a redelivered message after a slow-processing timeout). There, the "key" is typically the message's own unique identifier (an event ID already present in the payload, or `topic-partition-offset` when the producer doesn't supply one), and the "write" is the same insert-or-upsert-by-key operation, often performed in the same database transaction as the message's actual side effect so that both commit or neither does. [Kafka Delivery Semantics and Exactly-Once](../09-messaging-event-driven/delivery-semantics-and-exactly-once.md) covers the producer/broker-side half of this (idempotent producers, `acks=all`, exactly-once semantics scoped to Kafka-to-Kafka); this chapter's mechanism is the consumer-side half that makes an at-least-once-delivered message safe to apply more than once — the two are complementary pieces of the same end-to-end guarantee, not competing techniques.
...
**Related references.** [§ Core Concepts](#core-concepts); [Distributed Systems Failure Modes](../10-distributed-systems/distributed-systems-failure-modes.md).
