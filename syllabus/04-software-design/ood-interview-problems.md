...
| Vending machine (this chapter's second worked example) | Modeling an entity whose legal actions depend on its current state | The State pattern ([Design Patterns Applied (GoF in Production)](design-patterns-applied.md)) — an explicit state representation over scattered booleans |
