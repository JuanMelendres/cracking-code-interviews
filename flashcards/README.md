...
| 177 | [Distributed Locking and Fencing Tokens](distributed-locking-and-fencing-tokens.md) | T-2422 | 3 | `syllabus/10-distributed-systems/distributed-locking-and-fencing-tokens.md` |
...
| N47 | [Memory Hierarchy: Caches, RAM, and Virtual Memory](memory-hierarchy-caches-ram-and-virtual-memory.md) | T-2006 | 6 | `syllabus/01-computer-science-foundations/memory-hierarchy-caches-ram-and-virtual-memory.md` |

**Added 2026-09-20: Matrix and Grid Traversal Patterns**, a gap-audit addition to `03-data-structures-algorithms` (T-2121) — zero dedicated coverage anywhere of in-place matrix rotation, spiral traversal, or flood fill; [Heaps, Top-K, and K-Way Merge](heaps-top-k-and-k-way-merge.md) (T-2106) also gained 1 more card the same day for its own real Quickselect addition (see its own row above, N11).
...
**2026-09-11 (same day, follow-up) — 2 more cards, closing a real full-range gap.** User pushed further, asking whether the 2xx/3xx/4xx/5xx *ranges* themselves (not just individual codes) were actually covered. They weren't, structurally — the chapter's own status-code content was a flat list, with no explicit range framing and no representative member of the `3xx` range demonstrated at all beyond `304`. Closed same day: the chapter gained a real, working `GET /books/latest` endpoint (a genuinely computed `302 Found` redirect whose target changes as new books are created) and real, zero-code evidence for `415 Unsupported Media Type`, plus an explicit per-range reference table (`1xx`–`5xx`) distinguishing every code demonstrated with real evidence from every code covered only conceptually (with a named reason why, per this deliverable's own no-fabrication rule — `202`/`206` have no honest real demo available in a synchronous JSON CRUD API). Added 2 more cards (first-digit range meaning; `301` vs. `302` and why a computed redirect must not be permanent). **New total: 240 decks, 811 cards.**

**2026-09-12 — 6-deck backlog: the entire `22-ai-llm-engineering` domain gains flashcard decks.** Same discovery as `cheat-sheets/README.md`'s matching entry: this domain's all 6 chapters (added 2026-09-09/10) had zero flashcard deck, never covered by any prior batch. Closed in one pass, 3 cards each: `llm-api-integration-fundamentals` (T-2300 — statelessness/cost growth, tool_use semantics, 429 backoff), `rag-and-vector-databases` (T-2301 — paraphrase retrieval failure, normalization/distance-metric pitfall, when an ANN index helps), `embeddings` (T-2302 — dimension count and signal-to-noise, paraphrase/polysemy failure modes, cross-model comparability), `prompt-engineering-patterns` (T-2303 — wording vs. real parameter, system-role override resistance, temperature as a measured parameter), `agentic-workflows-and-tool-orchestration` (T-2304 — iteration-cap safety, sequential-vs-parallel speedup, dependent-call correctness), `llm-evaluation-and-testing` (T-2305 — exact-match false negatives, semantic-similarity's relocated false negative, what spot-checking misses). Every card's fact extracted directly from each chapter's own real, executed demo output. **New total: 246 decks (240 prior + 6), 829 cards (811 prior + 18).**

**2026-09-12 (same day, follow-up) — GraphQL and gRPC API Design (T-917/T-918) gain flashcard decks.** Same discovery pass, same two chapters as `cheat-sheets/README.md`'s matching entry: both had zero flashcard deck despite already carrying a real inline "Flashcards" section per the handbook-chapter template. 3 cards each, extracted verbatim from each chapter's own existing cards: `graphql-api-design` (N+1/DataLoader batching, null-propagation blast radius, HTTP 200-on-error), `grpc-api-design` (shared-codegen contract enforcement, the four call shapes by `stream` placement, deadline-less-call cascading failure). **New total: 248 decks (246 prior + 2), 835 cards (829 prior + 6).**
...
**2026-09-21 (same day, follow-up) — Distributed Locking and Fencing Tokens (T-2422) gains a flashcard deck.** New `10-distributed-systems` chapter closing a gap audit that found the consensus chapter's own "use a distributed lock" fix referenced twice but never explained. 3 cards. **New total: 268 decks (267 prior + 1), 931 cards (928 prior + 3).**

**2026-09-21 (same day, follow-up) — Authentication Attack Defense: Brute Force, Credential Stuffing, and MFA (T-1310) gains a flashcard deck.** New `12-security` chapter closing a gap audit that found `owasp-top-10-for-backend-services.md`'s own A07 routing table named "Authentication Failures" but no chapter anywhere covered brute force, credential stuffing, account lockout, or MFA. 3 cards. **New total: 269 decks (268 prior + 1), 934 cards (931 prior + 3).**
...
**2026-09-22 — System Design Patterns: Recognition and Quick-Reference Guide (T-2427) gains a flashcard deck.** New `11-system-design` chapter, user-requested. 3 cards. **Correction, found while adding the next entry below:** this deck's own dated note was never added when the file was created — table row 186 existed but this note-chain skipped straight past it. New total, verified directly against the file system: **281 decks, 982 cards**.
