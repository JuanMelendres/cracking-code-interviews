...
**Batches 3–4 converted the remaining 5 earlier-format rounds** (Weeks 1, 2, 4, 7, 8). These use a `Part A — Candidate script` / `Part B — Interviewer script` structure with no inline per-question pass/fail signals, and are scored in the source against a shared six-dimension rubric (`study-packs/week-01/10-week-1-evaluation-rubric.md`, either all six dimensions for Weeks 1/2/4 or just Technical Depth and Production Judgment for Weeks 7/8, per each source mock's own instruction) rather than a per-question 1–5 scale. Converting these into the Standard's evaluator structure required constructing pass/borderline/fail signals — genuinely heavier than the mechanical elevation done in batches 1–2 — but every signal is grounded in the source's own rubric evidence anchors and interviewer-script "listening for"/push-back cues, and in the real, measured content of the canonical `handbook/` chapters each round draws on, never invented independent of the source. The Week 4 round is also structurally different: a single 45-minute open-ended design prompt rather than a numbered question bank, so its Evaluator Section is organized around the six-phase design method plus the source's named interviewer probes instead of per-question numbering.

**This closed the Mock Interviews deliverable at 12/12 rounds (2026-08-11), against its own original scope — all 12 real study-pack mock-interview files elevated.**

**Round 13 added 2026-09-13.** A repository-wide freshness audit found this deliverable, like `study-packs/`, `cheat-sheets/`, and `production-cookbook/` before it, had never been re-checked against the domains added after its 12/12 closure — specifically `22-ai-llm-engineering` (added 2026-09-09/10), `07-api-design`'s GraphQL/gRPC advanced topics, and `21-frontend-web`'s newest chapters (F-401–F-403), none of which had a mock round. [AI/LLM Engineering Technical Round](ai-llm-engineering-technical-round.md), [GraphQL and gRPC API Design Round](graphql-grpc-api-design-round.md), and [Frontend Security, Real-Time UI, and Micro-Frontends Round](frontend-security-realtime-and-microfrontends-round.md) close all three of these gaps — each built without a prior study-pack mock source (all three domains postdate every relevant study-pack program), but not invented: every question is drawn from the relevant canonical chapter's own already-written Interview Questions or flashcard deck, restructured into this deliverable's Mock Interview Standard. **This closes the mock-interview freshness audit in full — no known outstanding gap remains.**
...
| 11 | [Spring Technical Round](spring-technical-round.md) | Bean lifecycle + proxy-creation hook, `@Async`+`@Transactional` visibility gotcha, security filter chain trace, honest JWT revocation, PKCE vs. client secret | `study-packs/week-07/07-week-7-mock-interview.md` |
| 12 | [Kafka Messaging Technical Round](kafka-messaging-technical-round.md) | Per-partition ordering, `acks=all` data-loss mechanism, repeated-rebalance diagnosis, honest exactly-once scoping, hot-partition remediation | `study-packs/week-08/08-week-8-mock-interview.md` |
| 13 | [AI/LLM Engineering Technical Round](ai-llm-engineering-technical-round.md) | Conversation-growth cost mechanic, RAG failure-mode diagnosis, cross-model embedding non-comparability, system/user role trust boundary, agentic-loop cost estimation, golden-dataset regression discipline | None — built from each canonical chapter's own Interview Questions section (no prior study-pack mock existed for this domain) |
| 14 | [GraphQL and gRPC API Design Round](graphql-grpc-api-design-round.md) | GraphQL N+1/`DataLoader`, null propagation, HTTP-200-on-error; gRPC call-shape selection, deadline-less cascading failure, shared-codegen contract guarantee | None — built from each chapter's own Interview Questions and flashcard deck (no prior study-pack mock existed for this domain) |
| 15 | [Frontend Security, Real-Time UI, and Micro-Frontends Round](frontend-security-realtime-and-microfrontends-round.md) | CSP's real scope vs. XSS, CSRF as a distinct problem, WebSocket reconnect-storm/backoff, WebSocket vs. EventSource auto-reconnection, Module Federation `shared`/`singleton`, proving independent deployability | None — built from each chapter's own Interview Questions (F-401–F-403, no prior study-pack mock existed for this domain) |
