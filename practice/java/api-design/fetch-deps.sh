#!/bin/bash
# Downloads the real Spring Framework (Spring MVC's real @RequestMapping
# dispatch), Spring Boot Test (real MockMvc), and JUnit 5 jars used by
# this pack's HATEOAS/RFC 9457/filtering-sorting/bulk-operations demos,
# directly from Maven Central -- no Maven/Gradle install required. Jars
# are gitignored (*.jar); run this before compiling.
set -e
cd "$(dirname "$0")"
mkdir -p lib
cd lib
