...
# Tomcat jars used by this pack's demos, directly from Maven Central -- no
# Maven/Gradle install required. Jars are gitignored (*.jar); run this before
# compiling. Versions resolved by walking spring-boot-starter-web's own real
# POM dependency tree (transitively), not guessed.
set -e
cd "$(dirname "$0")"
mkdir -p lib
cd lib
...
# Micrometer (required by spring-context observation support)
...
# Logging (Boot's default: Logback via SLF4J)
...
# Jackson (JSON, pulled in by spring-boot-starter-json)
...
# Embedded Tomcat -- THE mechanism this chapter's core demo measures
