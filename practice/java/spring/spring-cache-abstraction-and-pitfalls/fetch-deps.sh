#!/bin/bash
# Downloads the real Spring Framework jars used by this pack's caching demos,
# directly from Maven Central -- no Maven/Gradle install required, no Spring Boot
# auto-configuration masking the mechanism. Jars are gitignored (*.jar); run this
# before compiling.
set -e
cd "$(dirname "$0")"
mkdir -p lib
